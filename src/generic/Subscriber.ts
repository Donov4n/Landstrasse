...
export class Subscriber extends MessageProcessor {
    ...
    private subs = new PendingMap<WampSubscribedMessage>(
    ...
    public async Subscribe<A extends WampList, K extends WampDict>(
        ...
        return subscribedPromise.then((subscribed) => {
            ...
            if (!subscriptionWrapper) {
                subscriptionWrapper = new MultiSubscription(
                    subId,
                    topic,
                    async (sub) => await this.sendUnsubscribe(sub),
                );
                this.currentSubscriptions.set(subId, subscriptionWrapper);
            }
            return new Subscription(
                handler as EventHandler<WampList, WampDict>,
                requestID,
                subscriptionWrapper,
                this.logger,
            );
