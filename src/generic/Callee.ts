...
class Call {
        ...
        if (!res.nextResult || this.progress) {
            ...
            try {
                ...
                if (err instanceof SerializationError) {
                    this.logger.log(
                        LogLevel.WARNING,
                        `Serialization for ${this.callid} failed, sending error`,
                    );
                    await this.onHandlerError(
                        new WampError('wamp.error.serialization-error'),
                    );
                }
            }
    ...
    private async onHandlerError(err: any): Promise<void> {
        ...
        this.logger.log(
            LogLevel.DEBUG,
            `ID: ${this.callid}, Sending Error ${wampError.errorUri}`,
        );
...
export class Callee extends MessageProcessor {
    ...
    private regs = new PendingMap<WampRegisteredMessage>(
    ...
    >(
        ...
        const registration = new Registration(
            ...
            uri,
            handler as any,
            async (id) => await this.unregister(id),
        );
    ...
    protected onMessage(msg: WampMessage): boolean {
        ...
        if (msg[0] === EWampMessageID.INTERRUPT) {
            ...
            if (!call) {
                ...
                this.logger.log(
                    LogLevel.DEBUG,
                    `ID: ${requestID}, Received Cancellation Request`,
                );
                call.cancel();
            }
            return true;
